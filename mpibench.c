...
int main(int argc, char **argv) {
  void leastsquares(int h0, int h1, double *t, double *g, double *l);
  int p, s, s1, iter, i, n, h, *Nsend, *Nrecv, *Offset_send, *Offset_recv;
  double alpha, beta, x[MAXN], y[MAXN], z[MAXN], src[MAXH], dest[MAXH], time0,
      time1, time, *Time, mintime, maxtime, nflops, r, g0, l0, g, l,
      t[MAXH + 1];
    ...
    if (s == 0) {
      ...
      if (mintime > 0.0) {
        ...
        for (s1 = 0; s1 < p; s1++)
          r += nflops / Time[s1];
        r /= p;
        printf("n= %5d min= %7.3lf max= %7.3lf av= %7.3lf Mflop/s ", n,
               nflops / (maxtime * MEGA), nflops / (mintime * MEGA), r / MEGA);
        fflush(stdout);
  ...
  for (h = 0; h <= MAXH; h++) {
    ...
    time0 = MPI_Wtime();
    for (iter = 0; iter < NITERS; iter++) {
      MPI_Alltoallv(src, Nsend, Offset_send, MPI_DOUBLE, dest, Nrecv,
                    Offset_recv, MPI_DOUBLE, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
    }
    time1 = MPI_Wtime();
    time = time1 - time0;
