...
void mpimv(int p, int s, int n, int nz, int nrows, int ncols, double *a,
           int *inc, int *srcprocv, int *srcindv, int *destprocu, int *destindu,
  ...
  */
  ...
  for (j = 0; j < ncols; j++)
    MPI_Get(&vloc[j], 1, MPI_DOUBLE, srcprocv[j], srcindv[j], 1, MPI_DOUBLE,
            v_win);
  MPI_Win_fence(0, v_win);
  ...
  for (i = 0; i < nrows; i++) {
    *psum = 0.0;
    ...
    MPI_Accumulate(psum, 1, MPI_DOUBLE, destprocu[i], destindu[i], 1,
                   MPI_DOUBLE, MPI_SUM, u_win);
    pvloc -= ncols;
  }
  MPI_Win_fence(0, u_win);
...
void mpimv_init(int p, int s, int n, int nrows, int ncols, int nv, int nu,
  ...
  */
  ...
  MPI_Win_create(tmpprocv, np * SZINT, SZINT, MPI_INFO_NULL, MPI_COMM_WORLD,
                 &tmpprocv_win);
  MPI_Win_create(tmpindv, np * SZINT, SZINT, MPI_INFO_NULL, MPI_COMM_WORLD,
                 &tmpindv_win);
  MPI_Win_create(tmpprocu, np * SZINT, SZINT, MPI_INFO_NULL, MPI_COMM_WORLD,
                 &tmpprocu_win);
  MPI_Win_create(tmpindu, np * SZINT, SZINT, MPI_INFO_NULL, MPI_COMM_WORLD,
                 &tmpindu_win);

  MPI_Win_fence(0, tmpprocv_win);
  MPI_Win_fence(0, tmpindv_win);
  MPI_Win_fence(0, tmpprocu_win);
  MPI_Win_fence(0, tmpindu_win);
  ...
  for (j = 0; j < ncols; j++) {
    jglob = colindex[j];
    MPI_Get(&srcprocv[j], 1, MPI_INT, jglob % p, jglob / p, 1, MPI_INT,
            tmpprocv_win);
    MPI_Get(&srcindv[j], 1, MPI_INT, jglob % p, jglob / p, 1, MPI_INT,
            tmpindv_win);
  }
  for (i = 0; i < nrows; i++) {
    iglob = rowindex[i];
    MPI_Get(&destprocu[i], 1, MPI_INT, iglob % p, iglob / p, 1, MPI_INT,
            tmpprocu_win);
    MPI_Get(&destindu[i], 1, MPI_INT, iglob % p, iglob / p, 1, MPI_INT,
            tmpindu_win);
  }
  MPI_Win_fence(0, tmpprocv_win);
  MPI_Win_fence(0, tmpindv_win);
  ...
  MPI_Win_free(&tmpindu_win);
  MPI_Win_free(&tmpprocu_win);
  MPI_Win_free(&tmpindv_win);
  MPI_Win_free(&tmpprocv_win);
